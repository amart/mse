 ...
 model_data(argc,argv) , function_minimizer(sz)
{
  ...
  #endif
  age_3_plus_biomass_proj.allocate(endyr+1,endyr+nyrs_proj,"age_3_plus_biomass_proj");
  #ifndef NO_AD_INITIALIZE
    age_3_plus_biomass_proj.initialize();
  #endif
  estsrvbio_proj.allocate(1,nsel_srv,endyr+1,endyr+nyrs_proj,"estsrvbio_proj");
  #ifndef NO_AD_INITIALIZE
    estsrvbio_proj.initialize();
  #endif
  estsrvN_proj.allocate(1,nsel_srv,endyr+1,endyr+nyrs_proj,st_age,end_age,"estsrvN_proj");
  #ifndef NO_AD_INITIALIZE
    estsrvN_proj.initialize();
  #endif
  estsrvbio_proj_2_sd.allocate(endyr+1,endyr+nyrs_proj,"estsrvbio_proj_2_sd");
  #ifndef NO_AD_INITIALIZE
    estsrvbio_proj_2_sd.initialize();
  #endif
  estsrvbio_proj_2_multN.allocate(endyr+1,endyr+nyrs_proj,"estsrvbio_proj_2_multN");
  #ifndef NO_AD_INITIALIZE
    estsrvbio_proj_2_multN.initialize();
  #endif
  estFOFL_proj.allocate(endyr+1,endyr+nyrs_proj,"estFOFL_proj");
  #ifndef NO_AD_INITIALIZE
    estFOFL_proj.initialize();
  #endif
  estFABC_proj.allocate(endyr+1,endyr+nyrs_proj,"estFABC_proj");
  #ifndef NO_AD_INITIALIZE
    estFABC_proj.initialize();
  #endif
  estABC_proj.allocate(endyr+1,endyr+nyrs_proj,"estABC_proj");
  #ifndef NO_AD_INITIALIZE
    estABC_proj.initialize();
  #endif
  estABCaa_proj.allocate(endyr+1,endyr+nyrs_proj,st_age,end_age,"estABCaa_proj");
  #ifndef NO_AD_INITIALIZE
    estABCaa_proj.initialize();
  #endif
  mcmc_avg_rec.allocate("mcmc_avg_rec");
  #ifndef NO_AD_INITIALIZE
  mcmc_avg_rec.initialize();
  #endif
  mcmc_avg_log_rec.allocate("mcmc_avg_log_rec");
  #ifndef NO_AD_INITIALIZE
  mcmc_avg_log_rec.initialize();
  #endif
  mcmc_std_dev_avg_log_rec.allocate("mcmc_std_dev_avg_log_rec");
  #ifndef NO_AD_INITIALIZE
  mcmc_std_dev_avg_log_rec.initialize();
  #endif
  mcmc_CV_avg_log_rec.allocate("mcmc_CV_avg_log_rec");
  #ifndef NO_AD_INITIALIZE
  mcmc_CV_avg_log_rec.initialize();
  #endif
  mcmc_avg_rec_years.allocate("mcmc_avg_rec_years");
  #ifndef NO_AD_INITIALIZE
  mcmc_avg_rec_years.initialize();
  #endif
  mcmc_max_rec.allocate("mcmc_max_rec");
  #ifndef NO_AD_INITIALIZE
  mcmc_max_rec.initialize();
  #endif
  pad_mcmc_rng = new random_number_generator(1);;
  F100.allocate(endyr,endyr+nyrs_proj,"F100");
  #ifndef NO_AD_INITIALIZE
    F100.initialize();
  #endif
  SB100.allocate(endyr,endyr+nyrs_proj,"SB100");
  #ifndef NO_AD_INITIALIZE
    SB100.initialize();
  #endif
  SBtarget.allocate(endyr,endyr+nyrs_proj,"SBtarget");
  #ifndef NO_AD_INITIALIZE
    SBtarget.initialize();
  #endif
  F40.allocate(endyr,endyr+nyrs_proj,"F40");
  #ifndef NO_AD_INITIALIZE
    F40.initialize();
  #endif
  SB40.allocate(endyr,endyr+nyrs_proj,"SB40");
  #ifndef NO_AD_INITIALIZE
    SB40.initialize();
  #endif
  F35.allocate(endyr,endyr+nyrs_proj,"F35");
  #ifndef NO_AD_INITIALIZE
    F35.initialize();
  #endif
  SB35.allocate(endyr,endyr+nyrs_proj,"SB35");
  #ifndef NO_AD_INITIALIZE
    SB35.initialize();
  #endif
  F20.allocate(endyr,endyr+nyrs_proj,"F20");
  #ifndef NO_AD_INITIALIZE
    F20.initialize();
  #endif
  SB20.allocate(endyr,endyr+nyrs_proj,"SB20");
  #ifndef NO_AD_INITIALIZE
    SB20.initialize();
  #endif
  Sage_fsh_avg.allocate(st_age,end_age,"Sage_fsh_avg");
  #ifndef NO_AD_INITIALIZE
    Sage_fsh_avg.initialize();
  #endif
  M_at_age.allocate(st_age,end_age,"M_at_age");
  #ifndef NO_AD_INITIALIZE
    M_at_age.initialize();
  #endif
  ...
  SBcurr.initialize();
  #endif
  B0_mw.allocate(endyr+1,endyr+nyrs_proj,"B0_mw");
  #ifndef NO_AD_INITIALIZE
    B0_mw.initialize();
  #endif
  f.allocate(1,20,"f");
  ...
  sd_recruits_1.allocate(styr,endyr+1,"sd_recruits_1");
  sd_avg_rec_1.allocate("sd_avg_rec_1");
  sd_avg_rec_1_alt.allocate("sd_avg_rec_1_alt");
  sd_recruits_2.allocate(styr,endyr+1,"sd_recruits_2");
  sd_avg_rec_2.allocate("sd_avg_rec_2");
  sd_avg_rec_2_alt.allocate("sd_avg_rec_2_alt");
  sd_total_biomass.allocate(styr,endyr,"sd_total_biomass");
  sd_spawn_biomass.allocate(styr,endyr,"sd_spawn_biomass");
  sd_age_3_plus_biomass.allocate(styr,endyr,"sd_age_3_plus_biomass");
...
{
  obj_fun =0.0;
  random_number_generator& mcmc_rng= *pad_mcmc_rng;
...
void model_parameters::calculate_selectivity(void)
{
  random_number_generator& mcmc_rng= *pad_mcmc_rng;
    dvariable maxSel;
    int a,i,j;
...
void model_parameters::calculate_n_a_a(void)
{
  random_number_generator& mcmc_rng= *pad_mcmc_rng;
    ...
    for (y = styr; y <= endyr; y++)
    {
        ...
        for (a = st_age; a <= end_age; a++)
        {
            if (a < rcrage || a > trmage)
            {
                if (a > trmage && (complex_flag == 0 || complex_flag == 1 || complex_flag == 2))
                {
                    // separate q and same selectivity for survey 1 A, survey 1 B, and survey 1 C
                    i = 1;
        ...
        for (i = 1; i <= nsel_srv; i++)
        {
            // DDD wonders if elem_prod is doing what it is supposed to be doing
            ...
            if (totn > 0)
            {
                ...
                // accumulate older ages into BBB plus group
                if (end_age > trmage)
                {
                    for (j = (trmage + 1); j <= end_age; j++)
                    {
...
void model_parameters::calculate_c_a_a(void)
{
  random_number_generator& mcmc_rng= *pad_mcmc_rng;
    ...
    for (y = styr; y <= endyr; y++)
    {
        // ages covered by BBB model/data are 1-10;
...
void model_parameters::calculate_length_comps(void)
{
  random_number_generator& mcmc_rng= *pad_mcmc_rng;
    int uu,yy;
    pred_fsh_pal.initialize();
    ...
    // calculate survey 1 length composition
    ...
    // calculate survey 2 length composition
    ...
    // calculate survey 3 length composition
...
void model_parameters::calculate_bio_ref_points(dvar_vector sel, int curr_yr, dvar_vector Mvec)
{
  random_number_generator& mcmc_rng= *pad_mcmc_rng;
...
void model_parameters::evaluate_objective_function(void)
{
  random_number_generator& mcmc_rng= *pad_mcmc_rng;
    ...
    for (i = 1; i <= nyrsac_srv1; i++)      // survey 1
    {
        y = srv_acyrs1(i);
        // check if there is extended p-a-a data for this year
        extYear = false;
    ...
    for (i = 1; i <= nyrsac_srv2; i++)      // survey 2
    {
        y = srv_acyrs2(i);
        // check if there is extended p-a-a data for this year
        extYear = false;
    ...
    for (i = 1; i <= nyrsac_srv3; i++)      // survey 3
    {
        y = srv_acyrs3(i);
    ...
    for (i = 1; i <= nyrslen_srv1; i++)     // survey 1
    {
        for (j = 1; j <= nbins3; j++)
        {
    ...
    for (i = 1; i <= nyrslen_srv2; i++)     // survey 2
    {
        for (j = 1; j <= nbins2; j++)
        {
    ...
    for (i = 1; i <= nyrslen_srv3; i++)     // survey 3
    {
        for (j = 1; j <= nbins2; j++)
        {
    ...
    // likelihood function for expanded p-a-a data
    ...
    for (i = 1; i <= nyrs_srv_1_paa_all; i++)   // extended survey 1 p-a-a data
    {
    ...
    for (i = 1; i <= nyrs_srv_2_paa_all; i++)   // extended survey 2 p-a-a data
    {
    ...
    // likelihood component for scaling srv1 C q to B q
 ...
 adstring ad_tmp=initial_params::get_reportfile_name();
  ...
  if (!report)
  {
    ...
    if (last_phase())
    {
        ...
        // NOTE:  BBB uses (endyr-5) through (endyr-1)
        Sage_fsh_avg.initialize();
        Sage_fsh_avg = 0.0;
        for (a = st_age; a <= end_age; a++)
        {
    ...
    report << endl;
    report << "survey 1 A biomass estimates" << endl;
    for (i = 1; i <= nyrs_srv1_bs; i++)     // A survey (survey 1)
    {
        y = srvyrs1_bs(i);
    ...
    report << endl;
    report << "survey 1 B biomass estimates" << endl;
    for (i = 1; i <= nyrs_srv1_ek; i++)     // B survey (survey 1)
    {
        y = srvyrs1_ek(i);
    ...
    report << endl;
    report << "survey 1 C biomass estimates" << endl;
    for (i = 1; i <= nyrs_srv1_dy; i++)     // C survey (survey 1)
    {
        y = srvyrs1_dy(i);
...
void model_parameters::calculate_projections(void)
{
  random_number_generator& mcmc_rng= *pad_mcmc_rng;
    int uu,yy,y,a;
...
int model_parameters::get_iter_seed(int iter)
{
  random_number_generator& mcmc_rng= *pad_mcmc_rng;
    int ii;
    char s[80];
...
void model_parameters::compose_input_data_file(int curryr)
{
  random_number_generator& mcmc_rng= *pad_mcmc_rng;
    ...
    double srv5;
    int ii,uu;
    // use BBB's input data file (stock_assess.dat) as the estimator output file
    if (curryr == endyr)
    {
        // read the input file in once ONLY
        if (!readIDFfile)
        {
            // read in stock_assess.dat data file
            int nline = 0;
            int len_s;
            ifstream ifs("stock_assess.dat", ios::in);
    ...
    {
        // set stock_assess_proj.dat to stock_assess_proj.dat + this year's data
        ...
        // survey 1 (C survey) indices & CV, p-a-a, w-a-a,
        // survey 2 (trawl) indices & CV, p-a-a, w-a-a BIENNIALLY,
        // survey 3 (coastal) indices & CV, p-a-a, w-a-a
        ...
        rep4 = idf_proj.findNodeNum("#Number of C surveys");
        ...
        abf4 = idf_proj.findNodeNum("#Survey 2 data (trawl)");
        ...
        // abf7 = idf_proj.findNodeNum(adstring("#Survey 3 data (production)"));
        // abf8 = idf_proj.findNodeNum(adstring(""));
        // abf9 = idf_proj.findNodeNum(adstring(""));
        abfA = idf_proj.findNodeNum("#Survey 3 data (coastal)");
        ...
        // abfD = idf_proj.findNodeNum(adstring("#Survey 5 data (index)"));
        // abfE = idf_proj.findNodeNum(adstring(""));
        abfF = idf_proj.findNodeNum("#Age error transition (10 X 10)");
        abfG = idf_proj.findNodeNum("#Population weight at age (kg) at spawning (Use survey 1 estimates)");
...
dvariable model_parameters::calculate_F_proj(int curryr,dvar_vector Mvec)
{
  random_number_generator& mcmc_rng= *pad_mcmc_rng;
...
void model_parameters::calculate_pop_params(int curryr)
{
  random_number_generator& mcmc_rng= *pad_mcmc_rng;
...
void model_parameters::calculate_F_ABC(int curryr,dvar_vector Mvec)
{
  random_number_generator& mcmc_rng= *pad_mcmc_rng;
    int a, i;
    dvariable wt_a, wt_sp_a, spbio_tmp;
...
dvariable model_parameters::get_spr_rates(dvariable spr_percent, dvar_vector sel, int curryr, dvar_vector Mvec)
{
  random_number_generator& mcmc_rng= *pad_mcmc_rng;
    RETURN_ARRAYS_INCREMENT();
    double df=1.e-3;
...
dvariable model_parameters::spr_unfished(int curryr, dvar_vector Mvec)
{
  random_number_generator& mcmc_rng= *pad_mcmc_rng;
    RETURN_ARRAYS_INCREMENT();
    int jj, yy, nCVyrs;
...
dvariable model_parameters::spr_ratio(dvariable trial_F, dvar_vector sel, int curryr, dvar_vector Mvec)
{
  random_number_generator& mcmc_rng= *pad_mcmc_rng;
    RETURN_ARRAYS_INCREMENT();
...
dvector model_parameters::get_multinomial_sample(dvector obs_p, int st_idx, int end_idx, int nsamples)
{
  random_number_generator& mcmc_rng= *pad_mcmc_rng;
    RETURN_ARRAYS_INCREMENT();
    int i, j, nbin, ndraws;
...
int model_parameters::get_uniform_sample_int(int st_idx, int end_idx)
{
  random_number_generator& mcmc_rng= *pad_mcmc_rng;
    int isample = -1;
...
void model_parameters::write_curryr_params_to_file(int niter, int curryr)
{
  random_number_generator& mcmc_rng= *pad_mcmc_rng;
    ...
    if (curryr == (endyr + nyrs_proj))
    {
        ...
        // output parameters to compare with stock_assess.rep file contents
...
void model_parameters::write_idf_to_estimator_file(void)
{
  random_number_generator& mcmc_rng= *pad_mcmc_rng;
    int i;
    int nLines = idf_proj.countNodes();
    adstring s;
    ofstream ofs("stock_assess_proj.dat", ios::out);
...
void model_parameters::run_estimator_model(int niter, int curryr)
{
  random_number_generator& mcmc_rng= *pad_mcmc_rng;
    int retval = 0;
    char s[MAX_IDF_LINE_LEN];
...
dvariable model_parameters::parse_estimator_results(int niter, int curryr)
{
  random_number_generator& mcmc_rng= *pad_mcmc_rng;
    RETURN_ARRAYS_INCREMENT();
    ...
    if (catch_flag == 1 || catch_flag == 2)
    {
        ifstream erf("stock_assess.rep", ios::in);
        // go to the beginning of the file
        erf.seekg(0, ios::beg);
        ...
        {
            // cout << "in parse_estimator_results: s = " << s << endl;
            foundIt = (strncmp(s, "Values of biological reference points",37) == 0);
        }
        // if we found the projected fishing mortality, then parse it
        if (foundIt)
        {
...
{
  dvector temp("{1.e-1, 1.e-2, 1.e-3, 1.e-4, 1.e-5, 1.e-5, 1.e-6, 1.e-7}");
  convergence_criteria.allocate(temp.indexmin(),temp.indexmax());
  convergence_criteria=temp;
  dvector temp1("{1000, 1000, 1000, 2000, 2000, 2000, 5000, 5000}");
